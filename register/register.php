<?php
	...
	function generateUser($config,$pdo) {
		...
		// Create user account
		$stmt = $pdo->prepare('INSERT INTO Users (Username, UpperUser, Hash, Salt, Email, TimeJoined) VALUES (:user,:upperuser,:hash,:salt,:email,:joinTime);');
		...
		$stmt->bindParam(':joinTime', $joinTime, PDO::PARAM_STR); // <-- Automatically sanitized for SQL by PDO
		$stmt->execute();
